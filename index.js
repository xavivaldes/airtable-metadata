...
class Base {
  ...
  async retrieveTables() {
    ...
    this.tables = await service.retrieve().then(ss => ss.tables.map(s => new Table(s, service.r(s.id))));
    this.tablesMap = new Map(this.tables.reduce((a, t) => [...a, [t.schema.id, t]], []));
  }
  ...
  async createTable({name, fields = [{type: "singleLineText", name: "name"}]}) {
    return new Table(await performCall({
...
export class AirtableMetadata {
  constructor({endpoint = AIRTABLE_ENDPOINT, service = AIRTABLE_SERVICE_NAME, apiKey = AIRTABLE_API_KEY} = {}) {
  ...
  async listBases() {
    ...
    return baseService.retrieve().then(ss => ss.bases.map(s => new Base(s, service.r(s.id))));
  }
